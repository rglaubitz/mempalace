...
- **`mempalace_diary_read` silently dropped entries on agent-name case mismatch.** `tool_diary_write` stored the `agent` metadata verbatim after `sanitize_name`, which preserves case, while `tool_diary_read` filtered by exact match. Writing as `"Claude"` and reading as `"claude"` (or vice-versa) returned zero rows. Both endpoints now lowercase `agent_name` immediately after sanitization, so reads are case-insensitive and the default per-agent wing slug is stable across casings. **Behavior change:** entries written prior to this fix under mixed-case agent names are not auto-migrated — re-write the affected diary entries manually, or accept the loss. (A future `mempalace repair --mode diary-case` may automate this; tracked separately.) (#1243)
- **Compressed drawers now read through the renamed closets collection without losing legacy data.** `mempalace compress` writes to `mempalace_closets` so `palace.get_closets_collection()` and hybrid search share the same read path. Existing palaces that only have the old `mempalace_compressed` collection now fall back to that collection with a one-time warning. Operators may run a fresh `mempalace compress` to repopulate `mempalace_closets` before dropping the old collection. (#1244)
